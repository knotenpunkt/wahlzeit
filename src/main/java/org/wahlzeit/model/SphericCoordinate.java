...
public class SphericCoordinate extends AbstractCoordinate {
	...
	return Math.acos(Math.sin(this.latitude) * Math.sin(sc.getLatitude()) + Math.cos(this.latitude)
		* Math.cos(sc.getLatitude()) * Math.cos(Math.abs(sc.getLongitude() - this.longitude)));
	...
	if (latitude < 0 || latitude > (Math.PI * 2))
	    throw new IllegalArgumentException("latitude<0 OR latitude>2PI not allowed");
	...
	if (longitude < 0 || longitude > (Math.PI * 2))
	    throw new IllegalArgumentException("longitude<0 OR longitude>2PI not allowed");
